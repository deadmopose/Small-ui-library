...
local function create_window(window_info)
	...
	local tab_buttons_locked = false
	...
	local tabs_scrolling_frame = Instance.new("ScrollingFrame")
	...
	TextLabel.Position = UDim2.new(0.0189541429, 0, 0.14257431, 0)
	TextLabel.Size = UDim2.new(0, 255, 0, 21)
	TextLabel.ZIndex = 2
	...
	UICorner.Parent = tabs_frame

	tabs_scrolling_frame.Active = true
	tabs_scrolling_frame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	tabs_scrolling_frame.BackgroundTransparency = 1.000
	tabs_scrolling_frame.BorderColor3 = Color3.fromRGB(27, 42, 53)
	tabs_scrolling_frame.BorderSizePixel = 0
	...
	UIListLayout.Parent = tabs_scrolling_frame
	...
	UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		...
		if not tab_buttons_locked and tabs_scrolling_frame.Visible then
			tabs_scrolling_frame.Size = UDim2.new(1, 0, 0, math.clamp(UIListLayout.AbsoluteContentSize.Y, 0, 252))
		end
	end)
	...
	holder.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 and input.UserInputState == Enum.UserInputState.Begin then
			...
			local frame_start_position = holder.Position
			...
			local move_connection = mouse.Move:Connect(function()
				...
				tween_service:Create(holder, TweenInfo.new(0.04, Enum.EasingStyle.Linear), {Position = frame_start_position + delta}):Play()
			end)
	...
	function window_functions:new_tab(tab_info)
		-- Variables
		...
		UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			...
			if not tab_buttons_locked and ScrollingFrame.Visible then
		...
		button.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 and input.UserInputState == Enum.UserInputState.Begin and not tab_buttons_locked then
				-- Setup

				tab_buttons_locked = true
